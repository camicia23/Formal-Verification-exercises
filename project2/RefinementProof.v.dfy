...
module RefinementProof refines RefinementObligation {
  ...
  datatype PartitionLayer = PartitionLayer(c: Constants, v: Variables) {
      ...
      requires v.WF(c)
    {
      ...
      map id | id in v.hosts :: HostOwner(id) := v.hosts[id].key_val
    ...
    ghost function MessageMaps() : PartitionsByOwner
    {
      ...
      map msg | msg in v.network.inFlightMessages ::
        MessageOwner(msg) := (imap k | k in msg.payload :: msg.payload[k])
      /*}*/
    }
      ...
      ensures PartitionOwnerClaimsKey(owner, key)
    {
      reveal_AllPartitions();
      match owner {
        case HostOwner(id) => assert owner in HostMaps();
        case MessageOwner(msg) => assert owner in MessageMaps();
      }
      assert owner in HostMaps() || owner in MessageMaps();
      assert owner in AllPartitions();
      assert key in AllPartitions()[owner];
    }
  ...
  {
    /*{*/
    var partLayer := PartitionLayer(c, v);

    if partLayer.IsFullAndDisjoint()
    then
      AtomicKVSpec.Variables(partLayer.SpecView())
    else
      // doesn't happen if Inv(c, v)
      var result:AtomicKVSpec.Variables :| true; result  // Replace with your definition
    /*}*/
  }

  /*{*/
  // Prove 1st postcondition of RefinementInit
  lemma InvariantInit(c: Constants, v: Variables)
    requires v.WF(c)
    requires Init(c, v)
    ensures Inv(c, v)
  {
    assert v.WF(c);
    var partLayer := PartitionLayer(c, v);
    var part := partLayer.AllPartitions();
    partLayer.reveal_AllPartitions();
    reveal_IsDisjoint();
    forall key ensures SomeOwnerDefinesKey(part, key){
      assert OwnerDefinesKey(part, HostOwner(0), key);
    }
    assert Inv(c, v);
  }

  lemma UnioneOfEmptyMaps(partMap:PartitionsByOwner)
    requires forall k | k in partMap :: partMap[k] == EmptyMap()
    ensures DisjointMapUnion(partMap) == EmptyMap()
  {

  }

  // Tool for 2nd postcondition of RefinementInit
  lemma UnionWithOnlyOneFull(partMap:PartitionsByOwner, theOne:MapOwner)
    requires theOne in partMap
    requires forall k | k in partMap && k != theOne :: partMap[k] == EmptyMap()
    ensures DisjointMapUnion(partMap) == partMap[theOne]
  {
    if(partMap == map[]){
      // fine
    } else {
      var source := ArbitraryOwner(partMap);

      if(source == theOne) {
        UnioneOfEmptyMaps(MapRemoveOne(partMap, source));
      } else {
        // fine
      }
    }
  ...
  // Prove 2nd postcondition of RefinementInit
  lemma SpecInitInv(c:Constants, v:Variables)
    requires v.WF(c)
    requires Init(c, v)
    requires Inv(c, v)
    ensures AtomicKVSpec.Init(ConstantsAbstraction(c), VariablesAbstraction(c, v))
  {
    PartitionLayer(c, v).reveal_AllPartitions();
    UnionWithOnlyOneFull(PartitionLayer(c, v).AllPartitions(), HostOwner(0));
  }
  /*}*/
  ...
  /*{*/
  lemma NextPreservesInv(c:Constants, v:Variables, v':Variables, event:Event)
    requires Inv(c, v)
    requires Next(c, v, v', event)
    ensures Inv(c, v')
  {
    var step:Step :| NextStep(c, v, v', step, event);
    var id := step.id;
    ...
    match event {
      case Put(key, val) => {
        ...
        assert msgOps.recv.None?;
        assert forall k:MapOwner | k in part && k != HostOwner(id) :: part[k] == part'[k];


        forall key ensures SomeOwnerDefinesKey(part', key){
          if key in part[HostOwner(id)] {
            assert OwnerDefinesKey(part', HostOwner(id), key);
            assert SomeOwnerDefinesKey(part', key);
          } else {
            assert SomeOwnerDefinesKey(part, key) ==>
                (exists owner :: owner in part && owner != HostOwner(id));
            var owner :| owner in part' && key in part'[owner] && owner != HostOwner(id);
            assert OwnerDefinesKey(part', owner, key);
            assert SomeOwnerDefinesKey(part', key);
          }
        }
        // 1 cond
        assert partLayer'.PartitionIsFull();


        reveal_IsDisjoint();
        assert forall src1, src2 | src1 in part && src2 in part && src1 != src2 :: (part[src1].Keys !! part[src2].Keys);
        forall src1, src2 | src1 in part' && src2 in part' && src1 != src2
          ensures (part'[src1].Keys !! part'[src2].Keys) {
          if(src1 == HostOwner(id)){
            assert part[src1].Keys == part'[src1].Keys;
          } else if (src2 == HostOwner(id)){
            assert part[src2].Keys == part'[src2].Keys;
          } else {
            // none is HostOwner(id)
          }
        }
        // 2 cond
        assert IsDisjoint(partLayer'.AllPartitions());
      }
      case Get(key, val) => {
        assert msgOps.send.None?;
        assert msgOps.recv.None?;

        assert partLayer' == partLayer;
        // assert Inv(c, v');
      }
      case NoOp() => {
        if msgOps.send.None? && msgOps.recv.None?{
          assert v == v';
          assert Inv(c, v');
        } else if msgOps.send.Some? {
          var msg := msgOps.send.value;
          forall key ensures SomeOwnerDefinesKey(part', key) {
            assert PartitionLayer(c, v).PartitionIsFull() ==> IsFullPartitions(part);
            assert SomeOwnerDefinesKey(part, key);
            assert exists owner :: OwnerDefinesKey(part, owner, key);
            var owner :| OwnerDefinesKey(part, owner, key);
            if owner == HostOwner(id) {
              assert OwnerDefinesKey(part', owner, key) || OwnerDefinesKey(part', MessageOwner(msg), key);

            } else {
              assert OwnerDefinesKey(part', owner, key);
            }
          }
          assert forall key :: SomeOwnerDefinesKey(part', key);
          assert IsFullPartitions(part');
          // 1st cond
          assert PartitionLayer(c, v').PartitionIsFull();


          // Reformulation of IsDisjoint()
          reveal_IsDisjoint();
          var map_sent := msg.payload;
          var iset_key_sent := iset k | k in map_sent.Keys;

          forall src1:MapOwner, src2:MapOwner | src1 in part' && src2 in part' && src1 != src2
            ensures part'[src1].Keys !! part'[src2].Keys
          {
            if src1 == MessageOwner(msg) || src2 == MessageOwner(msg) {
              var other := if src1 == MessageOwner(msg) then src2 else src1;
              if other == HostOwner(id) {
                // message' keys disjoint from Host's keys
                assert v'.hosts[id].key_val.Keys == v.hosts[id].key_val.Keys - iset_key_sent;
                assert iset_key_sent !! v'.hosts[id].key_val.Keys;
              } else {
                // message's keys disjoint from other host's keys
                assert part'[other].Keys == part[other].Keys;
                assert part[HostOwner(id)] == v.hosts[id].key_val;
                assert part'[MessageOwner(msg)].Keys == iset k | k in msg.payload;
                assert forall k :: k in msg.payload ==> k in v.hosts[id].key_val;
                assert iset_key_sent <= v.hosts[id].key_val.Keys;
                assert part'[MessageOwner(msg)].Keys <= part[HostOwner(id)].Keys;
                assert iset_key_sent !! part'[other].Keys;
                assert part'[src1].Keys !! part'[src2].Keys;
              }
            } else {
              if src1 == HostOwner(id) || src2 == HostOwner(id) {
                var other := if src1 == HostOwner(id) then src2 else src1;
                assert part'[other].Keys == part[other].Keys;
                assert part'[HostOwner(id)].Keys <= part[HostOwner(id)].Keys;
                assert part'[src1].Keys !! part'[src2].Keys;
              } else {
                assert part'[src1] == part[src1];
                assert part'[src2] == part[src2];
                assert part'[src1].Keys !! part'[src2].Keys;
              }
            }
          }
          ...
          // 2nd cond
          assert IsDisjoint(PartitionLayer(c, v').AllPartitions());
          assert PartitionLayer(c, v').IsFullAndDisjoint();
          assert Inv(c, v');
        } else if msgOps.recv.Some? {
          // Receiving message...
          var msg := msgOps.recv.value;
          var iset_key_recv := iset k | k in msg.payload.Keys;
          ...
          forall key ensures SomeOwnerDefinesKey(part', key) {
            ...
            if owner == HostOwner(id) {
              assert OwnerDefinesKey(part', HostOwner(id), key);
            } else if owner == MessageOwner(msg) {
              assert OwnerDefinesKey(part', HostOwner(id), key);
            } else {
              assert OwnerDefinesKey(part', owner, key);
            }
          ...
          reveal_IsDisjoint();
          forall src1, src2
            | src1 in part' && src2 in part' && src1 != src2
            ensures part'[src1].Keys !! part'[src2].Keys {
            if src1 == HostOwner(id) || src2 == HostOwner(id) {
              var other := if src1 == HostOwner(id) then src2 else src1;
              assert other != MessageOwner(msg);
              assert part'[other].Keys == part[other].Keys;

              assert iset_key_recv <= part'[HostOwner(id)].Keys;
              assert part[HostOwner(id)].Keys <= part'[HostOwner(id)].Keys;
              assert iset_key_recv == part[MessageOwner(msg)].Keys;
          ...
          // 2nd cond
          assert IsDisjoint(part');
          assert partLayer'.IsFullAndDisjoint();
          assert Inv(c, v');
        } else {
          assert false;
        }
    ...
    // Definition of IsFullAndDisjoint:
    assert PartitionLayer(c, v').PartitionIsFull();
    assert IsDisjoint(PartitionLayer(c, v').AllPartitions());
  ...
  lemma SpecNextCorresponds(c: Constants, v: Variables, v': Variables, event: Event)
    requires Inv(c, v)
    requires Next(c, v, v', event)
    requires Inv(c, v')
    ensures AtomicKVSpec.Next(ConstantsAbstraction(c), VariablesAbstraction(c, v), VariablesAbstraction(c, v'), event)
  {
    var abstConst := ConstantsAbstraction(c);
    var abstVar := VariablesAbstraction(c, v);
    var abstVar' := VariablesAbstraction(c, v');
    ...
    var step :| NextStep(c, v, v', step, event);
    var id := step.id;
    var msgOps := step.msgOps;
    ...
    match event{
      case Get(key, val) => {
        assert partLayer == partLayer';
        assert partLayer.SpecView() == partLayer'.SpecView();
        assert abstVar == abstVar';
        assert AtomicKVSpec.Next(abstConst, abstVar, abstVar', event);
      }
      case Put(key, val) => {
        assert forall k :: (k in abstVar'.key_value <==> k in abstVar.key_value[key := val]);
        forall k | k in abstVar'.key_value || k in abstVar.key_value[key := val]
          ensures abstVar'.key_value[k] == abstVar.key_value[key := val][k] {
          if k == key {
            assert v'.hosts[id].key_val[k] == val;
            DisjointUnionMapsKeyToValue(part', HostOwner(id), key);
            assert abstVar'.key_value[k] == abstVar.key_value[key := val][k];
          } else {
            assert SomeOwnerDefinesKey(part', k);
            var k_owner :| OwnerDefinesKey(part', k_owner, k);
            DisjointUnionMapsKeyToValue(part, k_owner, k);
            DisjointUnionMapsKeyToValue(part', k_owner, k);
            assert abstVar'.key_value[k] == abstVar.key_value[key := val][k];
          }
        }
        assert abstVar'.key_value == abstVar.key_value[key := val];
        assert AtomicKVSpec.Next(abstConst, abstVar, abstVar', event);
      }
      case NoOp() => {
        if msgOps.send.Some? {
          ...
          var msg := msgOps.send.value;
          forall k:int
            ensures abstVar.key_value[k] == abstVar'.key_value[k] {
            if k in msg.payload.Keys {
              assert msg.payload[k] == part[HostOwner(id)][k];
              DisjointUnionMapsKeyToValue(part, HostOwner(id), k);
              DisjointUnionMapsKeyToValue(part', MessageOwner(msg), k);
              assert abstVar.key_value[k] == abstVar'.key_value[k];
            } else {
              assert SomeOwnerDefinesKey(part', k);
              var k_owner :| OwnerDefinesKey(part', k_owner, k);
              DisjointUnionMapsKeyToValue(part, k_owner, k);
          ...
          assert forall k :: (k in abstVar.key_value <==> k in abstVar'.key_value);
          assert abstVar == abstVar';
        } else if msgOps.recv.Some? {
          assert msgOps.send.None?;
          var msg := msgOps.recv.value;
          forall k:int
            ensures abstVar.key_value[k] == abstVar'.key_value[k] {
            if k in msg.payload.Keys {
              var msgOwner := MessageOwner(msg);
              var hostOwner := HostOwner(id);

              DisjointUnionMapsKeyToValue(part, msgOwner, k);
              assert abstVar.key_value[k] == part[msgOwner][k];
              assert part[msgOwner][k] == msg.payload[k];

              reveal_IsDisjoint();
              assert IsDisjoint(part);
              assert part[msgOwner].Keys !! part[hostOwner].Keys;
              assert v'.hosts[id].key_val[k] == msg.payload[k];

              DisjointUnionMapsKeyToValue(part', hostOwner, k);
              assert abstVar'.key_value[k] == part'[hostOwner][k];
  ...
  {
    /*{*/
    NextPreservesInv(c, v, v', event);
    SpecNextCorresponds(c, v, v', event);
    /*}*/
  }
}

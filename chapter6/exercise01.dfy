...
module RefinementProof refines RefinementTheorem {
  ...
  /*{*/
  ghost function SumHostsKeys(hosts:seq<Host.Variables>) : nat
    decreases |hosts|
  {
    if |hosts| == 0 then 0
    else hosts[0].keysInApron + SumHostsKeys(hosts[1..])
  }


  lemma SumHostKeysFirst(hosts:seq<Host.Variables>)
    requires forall i:nat | 0 < i < |hosts| :: hosts[i].keysInApron == 0
    ensures |hosts| == 0 || SumHostsKeys(hosts) == hosts[0].keysInApron
  {
  }

  lemma OneDifferenceSumHost(hosts:seq<Host.Variables>, hosts':seq<Host.Variables>, c:int, hostid:nat)
    requires hostid < |hosts|
    requires |hosts'| == |hosts|
    requires hosts'[hostid].keysInApron == hosts[hostid].keysInApron + c
    requires forall i:nat | i < |hosts| && i != hostid :: hosts[i] == hosts'[i]
    ensures SumHostsKeys(hosts') == SumHostsKeys(hosts) + c
  {
    if(hostid == 0){
      assert hosts'[1..] == hosts[1..];
      assert SumHostsKeys(hosts') == SumHostsKeys(hosts) + c;
    } else {
      OneDifferenceSumHost(hosts[1..], hosts'[1..], c, hostid-1);
      assert SumHostsKeys(hosts') == SumHostsKeys(hosts) + c;
    }
  }
  ...
  {
    /*{*/
    Spec.Variables(v.basket.numKeysInBasket + SumHostsKeys(v.hosts))
    /*}*/
  }
  ...
  {
    /*{*/
    c.totalKeys == v.customerPool.keysHeldByAllClients + v.basket.numKeysInBasket + SumHostsKeys(v.hosts)
    /*}*/
  }
  ...
  {
    /*{*/
    SumHostKeysFirst(v.hosts);
    assert c.totalKeys == SumHostsKeys(v.hosts);
    assert Inv(c, v);
    assert Spec.Init(ConstantsAbstraction(c), VariablesAbstraction(c, v));
    /*}*/
  }
    ...
    //        ensures Spec.Next(ConstantsAbstraction(c), VariablesAbstraction(c, v), VariablesAbstraction(c, v'), evt) || (VariablesAbstraction(c, v) == VariablesAbstraction(c, v') && evt == NoOp)
  {
    /*{*/
    if evt == Acquire {
      assert exists step :: DistributedSystem.NextStep(c, v, v', evt, step);
      var step :| DistributedSystem.NextStep(c, v, v', evt, step);

      OneDifferenceSumHost(v.hosts, v'.hosts, -1, step.hostid);

      assert c.totalKeys == v'.customerPool.keysHeldByAllClients + v'.basket.numKeysInBasket + SumHostsKeys(v'.hosts);
    } else if evt == Release {
      assert exists step :: DistributedSystem.NextStep(c, v, v', evt, step);
      var step :| DistributedSystem.NextStep(c, v, v', evt, step);
      ...
      OneDifferenceSumHost(v.hosts, v'.hosts, -step.basketOp.keyDelta, step.hostid);

      assert c.totalKeys == v'.customerPool.keysHeldByAllClients + v'.basket.numKeysInBasket + SumHostsKeys(v'.hosts);
    }
    assert c.totalKeys == v'.customerPool.keysHeldByAllClients + v'.basket.numKeysInBasket + SumHostsKeys(v'.hosts);
    assert Inv(c, v');
    assert Spec.Next(ConstantsAbstraction(c), VariablesAbstraction(c, v), VariablesAbstraction(c, v'), evt) || (VariablesAbstraction(c, v) == VariablesAbstraction(c, v') && evt == NoOp);
    /*}*/
  }
}

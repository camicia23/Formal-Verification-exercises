...
/*{*/
// chopsticks[i] = -1 se appartiene a i-1
//                  0 se non ce l'ha nessuno
//                  1 se ce l'ha i
datatype Variables = AcquiredChopsticks(chopsticks:seq<int>)
{
  ghost predicate WellFormed(c: Constants) {
    && c.tableSize == |chopsticks|
    && forall i | 0 <= i < |chopsticks| :: chopsticks[i] == -1 || chopsticks[i] == 0 || chopsticks[i] == 1
  }
}
/*}*/
...
ghost predicate Init(c:Constants, v:Variables) {
  /*{*/
  match v
  case AcquiredChopsticks(chopsticks) =>
    && c.WellFormed()
    && v.WellFormed(c)
    && forall i:nat | i < c.tableSize :: chopsticks[i] == 0
  /*}*/
}
...
ghost predicate AcquireLeft(c:Constants, v:Variables, v':Variables, philosopherIndex:nat) {
  ...
  case AcquiredChopsticks(chopsticks) =>
    ...
    case AcquiredChopsticks(chopsticks') =>
      ...
      && chopsticks[philosopherIndex] == 0
      && chopsticks' == chopsticks[philosopherIndex := 1]
  /*}*/
}
...
ghost predicate AcquireRight(c:Constants, v:Variables, v':Variables, philosopherIndex:nat) {
  ...
  case AcquiredChopsticks(chopsticks) =>
    ...
    case AcquiredChopsticks(chopsticks') =>
      ...
      && v'.WellFormed(c)
      && philosopherIndex < c.tableSize
      && chopsticks' == chopsticks[(philosopherIndex+1)%c.tableSize := -1]
  /*}*/
}
...
ghost predicate ReleaseBoth(c:Constants, v:Variables, v':Variables, philosopherIndex:nat) {
  ...
  case AcquiredChopsticks(chopsticks) =>
    ...
    case AcquiredChopsticks(chopsticks') =>
      ...
      && v'.WellFormed(c)
      && philosopherIndex < c.tableSize
      && chopsticks[philosopherIndex] == 1
      && chopsticks[(philosopherIndex+1) % c.tableSize] == -1
      && chopsticks' == (chopsticks[philosopherIndex := 0])[(philosopherIndex+1) % c.tableSize := 0]
  /*}*/
}

datatype Step =
    /*{*/
  | acquireLeftStep(pos: nat)
  | acquireRightStep(pos: nat)
  | releaseStep(pos: nat)
/*}*/
...
ghost predicate NextStep(c:Constants, v:Variables, v':Variables, step: Step) {
  match step
  /*{*/
  case acquireLeftStep(pos) => AcquireLeft(c, v, v', pos)
  case acquireRightStep(pos) => AcquireRight(c, v, v', pos)
  case releaseStep(pos) => ReleaseBoth(c, v, v', pos)
  /*}*/
}
  ...
  requires v.WellFormed(c)
{
  ...
  match v
  case AcquiredChopsticks(chopsticks) =>
    forall i:nat | i < |chopsticks| :: chopsticks[i] == 0
  /*}*/
}
...
{
  /*{*/
  var bobo:Variables := AcquiredChopsticks([0, 0, 0]);
  var bobo2:Variables := AcquiredChopsticks([0, 1, 0]);
  var bobo3:Variables := AcquiredChopsticks([0, 1, -1]);
  behavior := [bobo, bobo2, bobo3];


  var step:Step := acquireLeftStep(1);
  assert NextStep(c, behavior[0], behavior[1], step);

  step := acquireRightStep(1);
